...
export const useLoginStore = defineStore('LoginStore', {
    ...
    actions: {
        async sendLoginData(data: Login) {
            ...
            try {
                const resp = await useApi(true)
                    .post<{ data: Login }, SessionResponseSuccess>(endpointUrl, {data: data});
                ...
                if (axios.isAxiosError(error) && error.response?.status === 422) {
                    ...
                    this.invalidFields = errorResponse.data.errors
                            ...
                            .map<string>(pointer => (pointer as string).replace(/^\/data\/attributes\//, ''));
                }
                throw error;
            } finally {
        ...
        async resendLoginData() {
            ...
            try {
                await useApi(true)
                    .post<{ data: Login }, SessionResponseSuccess>(endpointUrl, {data: this.login});
            } finally {
                this.isLoading = false;
            }
        },
